...
class StableDiffusionLoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_weights(
        ...
        **kwargs,
        ...
        is_correct_format = all(
            "lora" in key or "dora_scale" in key or "eigenlora" in key
    ...
    def load_lora_into_text_encoder(
        ...
        from peft import LoraConfig, EigenLoRAConfig
...
class StableDiffusionXLLoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_weights(
        ...
        **kwargs,
        ...
        use_eigenlora = kwargs.pop("use_eigenlora", None)
        ...
        **kwargs,
        ...
        if all(
            (
                ...
                or k.startswith("lora_te1_")
                or k.startswith("lora_te2_")
                or k.startswith("eigenlora_te_")
                or k.startswith("eigenlora_unet_")
                or k.startswith("eigenlora_te1_")
                or k.startswith("eigenlora_te2_")
            )
            for k in state_dict.keys()
        ):
            # Map SDXL blocks correctly.
        ...
        if any(cls.text_encoder_name in key for key in keys):
            ...
            if len(text_encoder_lora_state_dict) > 0:
                ...
                if use_eigenlora == True:
                    ...
                    for name, _ in text_encoder_attn_modules(text_encoder):
                        for module in ("out_proj", "q_proj", "k_proj", "v_proj"):
                            rank_key = f"{name}.{module}.eigenlora_B.loadings"
                            component_key = f"{name}.{module}.eigenlora_B.components"
                            if rank_key not in text_encoder_lora_state_dict:
                                continue
                    ...
                    eigenlora_config_kwargs = get_peft_kwargs(
                        rank,
                        num_components,
                        None,
                        text_encoder_lora_state_dict,
                    ...
                    eigenlora_config = EigenLoRAConfig(**eigenlora_config_kwargs)
...
class FluxLoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_into_transformer(
        ...
        if len(state_dict.keys()) > 0:
            ...
            if use_eigenlora == True:
                first_key = next(iter(state_dict.keys()))
                if "eigenlora_A" not in first_key:
                    state_dict = convert_unet_state_dict_to_peft(state_dict)
                ...
                rank = {}
                for key, val in state_dict.items():
                    if "eigenlora_B.loadings" in key:
                        rank[key] = val.shape[1]
                num_components = {}
                for key, val in state_dict.items():
                    if "eigenlora_B.components" in key:
                ...
                adapter_config = EigenLoRAConfig(**eigenlora_config_kwargs)
            elif use_eigenlora == False:
                first_key = next(iter(state_dict.keys()))
                if "lora_A" not in first_key:
                    state_dict = convert_unet_state_dict_to_peft(state_dict)
