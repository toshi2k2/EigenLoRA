"""
Utility functions for EigenLoRA computation.

This module provides functions for:
- Combining multiple LoRA adapters
- Computing eigenvectors from LoRA weight matrices
- Calculating EigenLoRA components and loadings
...
def calculate_eigenloras(
    ...
    """
    Calculate EigenLoRA components and optionally loadings from eigenvectors.

    This function projects LoRA weights onto the principal eigenvector subspace
    to create EigenLoRA components. If loadings=True, it also computes the
    ...
    Returns:
        State dictionary containing EigenLoRA components and optionally loadings.
    ...
    for k in lora_sd.keys():
        if "lora_A" in k:
            ...
            if loadings:
                ...
                eigenlora_sd.update({new_key_l: loading_vals})
        elif "lora_B" in k:
            components = nn.Parameter(
                eigenvectors[k]["eigenvectors"][:, :num_components]
            ).contiguous()
...
def add_classifier(
    eigenlora_dict: Dict[str, torch.Tensor],
    lora_dict: Dict[str, torch.Tensor],
) -> Dict[str, torch.Tensor]:
    """
    Add classifier weights from a LoRA state dict to an EigenLoRA state dict.

    Args:
        eigenlora_dict: EigenLoRA state dictionary to update.
        lora_dict: LoRA state dictionary containing classifier weights.

    Returns:
        Updated EigenLoRA state dictionary with classifier weights added.
    """
...
def add_gram_schmidt_vectors(
    ...
    """
    Extend EigenLoRA components with random orthogonal vectors.
    ...
    This increases the expressiveness of the EigenLoRA subspace.

    Args:
        state_dict: EigenLoRA state dictionary containing component matrices.
    ...
    Example:
        >>> eigenlora_sd = add_gram_schmidt_vectors(eigenlora_sd, 32)
